...
A guide to setting up automated pull request reviews that catch real issues without generating noise.
...
| **Impact** | Catches logic errors, security issues, and edge cases before human review |
...
Not every PR needs a full review. Configure filters to match the depth of review to the risk of the change.
...
For handling review feedback on your own PRs, use the triage pattern:
...
- `src/auth/login.ts:47` -- SQL query built via string concatenation.
  ...
  Suggested: `db.query('SELECT * FROM users WHERE email = $1', [email])`
...
- `src/api/users.ts:123` -- Missing authorization check on DELETE endpoint.
  ...
  Add: `if (req.user.id !== userId && !req.user.isAdmin) return 403`

### Medium
- `src/services/cache.ts:89` -- Cache TTL set to 24 hours for user session data.
  If a user's permissions change, stale cache could grant access for up to 24h.
...
- New auth endpoints in `src/auth/login.ts` have no test file.
  Add tests for: valid login, invalid password, rate limiting, SQL injection attempt.
...
- [DEV Community: Better PR Review Prompts](https://dev.to/yeahiasarker/code-review-ai-prompts-how-to-get-better-pull-request-reviews-from-ai-1mad)
